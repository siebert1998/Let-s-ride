...
import { fetchRouteBySlot, incrementRouteDownloadCount, upsertRouteForSlot } from '../services/routes';
...
export function RouteCard({ title, storageId, initialNotes = '', canEditRoutes = true }: RouteCardProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    const loadRoute = async (): Promise<void> => {
      ...
      try {
        ...
        setDownloadCount(syncedRoute.downloadCount ?? 0);
  ...
  const handleDownload = async (): Promise<void> => {
    ...
    try {
      const nextCount = await incrementRouteDownloadCount(storageId);
      setDownloadCount(nextCount);
    } catch {
      // Keep download flow usable if counter update fails.
    }
  };
  ...
  return (
    <article className="flex min-h-[420px] flex-col rounded-xl2 border border-line/80 bg-panel/95 p-4 shadow-card">
      ...
      <div className="mt-2 inline-flex items-center gap-2 text-xs font-semibold text-textMain">
        <span className="h-2 w-2 rounded-full bg-accent" aria-hidden="true" />
        <span>{downloadCount} rijders</span>
      </div>
    </article>
  );
}
